...
func (c *cache) goCaching() {
	go func() {
		...
		if err != nil {
			logger.Error("unable to get proxmox cluster", err)
		...
		for {
			...
			fmt.Printf("'%v'", rs)
			if err != nil {
				logger.Error("unable to get proxmox vm list", err)
				c.crashed.Store(true)
				return
			}
...
func (c *cache) get(fqnd string) (ip net.IP, ok bool) {
	if c.crashed.Load() {
		logger.Error("proxmox cache crashed. unable handle request")
		return nil, false
	}
...
func (c *cache) goUpdateQemu(r *proxmox.ClusterResource) {
	go func() {
		...
		if err != nil {
			logger.Error("unable to get vm interfaces", err)
			return
		}
		...
		for _, iface := range ifaces.Result {
			for _, addr := range iface.IPAddresses {
				...
				if c.net.Contains(ip) && !c.c[fqnd].Equal(ip) {
					c.mtx.Lock()
					c.c[fqnd] = ip
					fmt.Printf("updated a record, vm: %s, ip: %s", r.Name, addr.IPAddress)
					c.mtx.Unlock()
				}
			}
		}
